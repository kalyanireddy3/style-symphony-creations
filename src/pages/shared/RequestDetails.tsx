...
import { 
  ...
  mockLogout, 
  mockGetRequestById, 
  mockGetProposalsByRequestId,
  mockSubmitProposal,
  mockUpdateProposalStatus,
  mockGetTimelineByRequestId,
  mockAddTimelineUpdate
} from '@/services/mockData';
import { ProjectRequest, User, Proposal, TimelineUpdate } from '@/types';
...
const RequestDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const requestData = await mockGetRequestById(id);
        ...
        // Fetch proposals
        const proposalsData = await mockGetProposalsByRequestId(id);
        setProposals(proposalsData);
        
        // Fetch timeline
        const timelineData = await mockGetTimelineByRequestId(id);
  ...
  const handleSubmitProposal = async (proposalData: {
    ...
    try {
      ...
      const newProposal = await mockSubmitProposal(request.id, proposalData);
  ...
  const handleAcceptProposal = async (proposalId: string) => {
    try {
      await mockUpdateProposalStatus(proposalId, 'accepted');
  ...
  const handleRejectProposal = async (proposalId: string) => {
    try {
      await mockUpdateProposalStatus(proposalId, 'rejected');
      
      // Update local state
      setProposals(prev => prev.map(prop => 
        prop.id === proposalId ? { ...prop, status: 'rejected' } : prop
      ));
  ...
  const handleAddTimelineUpdate = async (status: TimelineUpdate['status'], message: string) => {
    ...
    try {
      const newUpdate = await mockAddTimelineUpdate(request.id, status, message);
        ...
        <div className="flex flex-col md:flex-row justify-between mb-6">
          <div>
            ...
            <div className="flex items-center mt-2">
              <Badge className={`${getStatusColor(request.status)} capitalize mr-2`}>
                {request.status}
              </Badge>
