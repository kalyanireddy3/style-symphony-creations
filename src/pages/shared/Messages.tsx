...
const Messages = () => {
    ...
    const checkAuthAndFetchData = async () => {
      try {
        ...
        if (partners.length > 0) {
          ...
          const initialMessages = await mockGetMessagesByUsers(userData.id, partners[0].user.id);
  ...
  useEffect(() => {
    const loadMessages = async () => {
      ...
      try {
        const messagesData = await mockGetMessagesByUsers(user.id, selectedPartner.id);
